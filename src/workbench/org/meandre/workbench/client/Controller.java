...
public class Controller {
	...
	/**
	 * A boolean value that indicates if a connection is currently being being
	 * created.
	 */
	...
	private void setNoDraw(boolean b) {
		_noDraw = b;
	}

	private boolean getNoDraw() {
		return _noDraw;
	}
	...
	void buildPublicRepoMaps(WBCommand followOnCMD) {
		new CommandGetPublicComponents(this, new CommandGetPublicFlows(this,
	 ...
	 *            AsyncCallback Callback object returned from the server.
	 */
	public void startInteractiveExecution(String sid, String execid,
			String flowid, AsyncCallback cb) {
	 ...
	 *            AsyncCallback Callback object returned from the server.
	 */
	public void updateInteractiveExecution(String sid, String execid,
			AsyncCallback cb) {
	 ...
	 * @param url
	 *            String URL of server to connect with.
	 * @param cb
	 *            AsyncCallback Callback object returned from the server.
	 */
	...
	HorizontalPanel buildButtPan() {
		...
		buttPan
				.setCellHorizontalAlignment(userlab,
				...
				new ClickListener() {
					public void onClick(Widget sender) {
						if (canvasHasComps() && isDirty()) {
							if (Window
									.confirm("Are you sure you want to exit? Your flow will not be saved.")) {
								_main.closeApp();
							}
						} else {
							_main.closeApp();
	 ...
	 *            Widget Component to be designated as draggable.
	 * @param comp
	 *            Widget Component to be used as the object that is used to
	 *            activate dragging.
	 */
	 ...
	 * @param pc
	 *            PortComp The "from" port of the connection under construction.
	 */
	...
	String buildComponentDescription(WBComponent ecd, boolean incRights) {
		...
		s += "</td></tr>" + "<tr><td>&nbsp;</td><td>&nbsp;</td></tr>"
				+ "<tr><td>Runnable:</td><td>" + ecd.getRunnable()
				+ "</td></tr>" + "<tr><td>Format:</td><td>" + ecd.getFormat()
				+ "</td></tr>" + "<tr><td>Location:</td><td>"
				+ ecd.getLocation() + "</td></tr>"
		...
		s += "</table></td></tr>" + "<tr><td>&nbsp;</td><td>&nbsp;</td></tr>"
				+ "</table>" + "</p>" + "</div>";
	...
	String buildFlowDescription(WBFlow flow, boolean incRights) {
		...
		String s = "<div id=\"componentshow\">" + "<h3>" + flow.getName()
				+ "</h3>" + "<p>" + "<table>"
		...
		s += "<tr><td><em>Creator:</em></td><td>"
				+ flow.getCreator()
				+ "</td></tr>"
				+ "<tr><td><em>Date:</em></td><td>"
		...
		if (secid.size() > 0) {
			for (Iterator itty = secid.iterator(); itty.hasNext();) {
				...
				s += "<tr><td>Component:</td><td>"
						+ ecid.getExecutableComponent().toString()
						+ "</td></tr>";
				...
				if (pd.getKeys().size() > 0) {
					s += "<tr><td valign=\"top\">Properties:</td><td valign=\"top\"><table>";
					for (Iterator itty2 = pd.getKeys().iterator(); itty2
							.hasNext();) {
	...
	/**
	 * Clear the canvas of all components and connections and reset all state in
	 * the controller.
	 */
	void clearCanvas() {
		/**
	...
	void deleteFlow() {
		...
		if (tp.getTabBar().getSelectedTab() != 1) {
			...
			if (ti == null) {
				...
				if (Window
						.confirm("Are you certain that you want to delete this flow from the repository?")) {
					if ((this._workingFlow != null)
							&& (this._workingFlow.getFlowID().trim()
									.equals(flow.getFlowID().trim()))) {
						this.clearCanvas();
						_main.getCompDescScrollPanel().clear();
					}
					new CommandDeleteFlow(this._repquery, this, null)
							.execute(flow);
				}
			}
		}
	...
	void removeLocation() {
		...
		if (tp.getTabBar().getSelectedTab() != 2) {
			...
			if (ti == null) {
				...
				if (Window
						.confirm("Are you certain that you want to remove this location from the repository?")) {
					if (this.isDirty()) {
	...
	void publishComponent() {
		...
		if (tp.getTabBar().getSelectedTab() != 0) {
			...
			if (ti == null) {
				...
				if (Window
						.confirm("Are you certain that you want to publish this component?")) {
					this.showStatusBusy();
					this.setStatusMessage("Publishing component "
							+ comp.getID());
	...
	void unpublishComponent() {
		...
		if (tp.getTabBar().getSelectedTab() != 0) {
			...
			if (ti == null) {
				...
				if (Window
						.confirm("Are you certain that you want to unpublish this component?")) {
					this.showStatusBusy();
					this.setStatusMessage("Unpublishing component "
							+ comp.getID());
	...
	void publishFlow() {
		...
		if (tp.getTabBar().getSelectedTab() != 1) {
			...
			if (ti == null) {
				...
				if (Window
						.confirm("Are you certain that you want to publish this flow?")) {
					this.showStatusBusy();
					this
							.setStatusMessage("Publishing flow "
									+ flow.getFlowID());
					new CommandPublishFlow(this, new CommandGetPublicFlows(
							this, new WBCommand() {
								public void execute(Object work) {
									Controller.this
											.regenerateTabbedPanel(false);
									Controller.this.getMain().getTabPanel()
											.selectTab(1);
								}
	...
	void unpublishFlow() {
		...
		if (tp.getTabBar().getSelectedTab() != 1) {
			...
			if (ti == null) {
				...
				if (Window
						.confirm("Are you certain that you want to unpublish this flow?")) {
					this.showStatusBusy();
					this.setStatusMessage("Unpublishing flow "
							+ flow.getFlowID());
					new CommandUnpublishFlow(this, new CommandGetPublicFlows(
							this, new WBCommand() {
								public void execute(Object work) {
									Controller.this
											.regenerateTabbedPanel(false);
	...
	void removeComponent(ComponentPanel cp) {
		...
		for (Iterator iter = cp.getInputs().iterator(); iter.hasNext();) {
			PortComp pc = (PortComp) iter.next();
			for (Iterator itty = pc.getConnections().iterator(); itty.hasNext();) {
				PortConn conn = (PortConn) itty.next();
				removeConnection(conn);
			}
		}
		for (Iterator iter = cp.getOutputs().iterator(); iter.hasNext();) {
			PortComp pc = (PortComp) iter.next();
			for (Iterator itty = pc.getConnections().iterator(); itty.hasNext();) {
				PortConn conn = (PortConn) itty.next();
				removeConnection(conn);
	...
	void saveFlow(boolean saveas) {
		...
		_workingFlow = flow;
		new CommandBuildFlow(_canvasComps, _connections, new CommandSaveFlow(
				_repquery, this, null)).execute(flow);
		_dirty = false;
	}
	...
	void saveFlowAndExecute() {
		...
		if (!_dirty) {
			...
			new CommandBuildFlow(_canvasComps, _connections,
					new CommandSaveFlow(_repquery, this,
							new CommandExecuteFlow(this, null))).execute(flow);
			_dirty = false;
		}
	}
	...
	void regenerateRepository() {
		if (Window
				.confirm("All unpublished components and flows will be deleted.  Are you certain that you want to regenerate the repository?")) {
			if (_dirty) {
				...
				AsyncCallback callback = new AsyncCallback() {
					public void onSuccess(Object result) {
						...
						if (cbo.getSuccess()) {
							...
							Controller.this
									.setStatusMessage("Repository regenerated successfully.");
						} else {
							Controller.this.hideStatusBusy();
							Controller.this.setStatusMessage("");
							Window
									.alert("Repository regeneration operation was NOT successful: "
											+ cbo.getMessage());
						}
					}
					...
					public void onFailure(Throwable caught) {
						...
						Controller.this.setStatusMessage("");
						Window
								.alert("AsyncCallBack Failure -- regeneratRepository:  "
										+ caught.getMessage());
					}
				};
	 ...
	 * @param x
	 *            int New component top position.
	 * @param y
	 *            int New component left position
	 * @param redraw
	 *            boolean Canvas redraw switch.
	 */
	void moveComponent(ComponentPanel cp, int left, int top, boolean redraw) {
	...
	void addFlowToCanvas(WBFlow flow) {
		...
		for (Iterator iter = flow.getExecutableComponentInstances().iterator(); iter
				.hasNext();) {
			...
			WBComponentInstance cinew = new WBComponentInstance(ci
					.getExecutableComponentInstance(), comp, ci.getName(), ci
					.getDescription(), new WBProperties(new java.util.HashMap(
					ci.getProperties().getValuesMap())));
		...
		setNoDraw(true);
		for (Iterator iter = flow.getConnectorDescriptions().iterator(); iter
				.hasNext();) {
    ...
    void addComponentToCanvas(WBComponent comp) {
		...
		WBComponentInstance ci = new WBComponentInstance(comp.getID() + "/"
				...
				comp, comp.getName(), comp.getDescription(), new WBProperties(
						new java.util.HashMap(comp.getProperties()
								.getValuesMap())));
	...
	PortConn makeConnection(PortComp from, PortComp to) {
		...
		from.setConnected(true, pc);
		to.setConnected(true, pc);
		_connections.add(pc);
		drawConnections();
		_dirty = true;
		return pc;
	...
	void removeConnection(PortConn conn) {
		conn.getFrom().setConnected(false, conn);
		conn.getTo().setConnected(false, conn);
	...
	/**
	 * Clears the canvas and redraws all the connections between components.
	 */
...
//				+ s_PROXY_TARGET_KEY + "=" + URL.encodeComponent(webUI);
					...
					AsyncCallback callback = new AsyncCallback() {
						public void onSuccess(Object result) {
							...
							if (items.size() == 0) {
								...
								for (Iterator itty = items.iterator(); itty
										.hasNext();) {
									...
									if (obj instanceof WBComponent) {
										...
										WBFlow f = (WBFlow) obj;
										String fname = (f.getName() != null) ? f
												.getName()
												: "";
										String flowid = (f.getFlowID() != null) ? f
												.getFlowID()
												: "";
										String flowdesc = (f.getDescription() != null) ? f
												.getDescription()
												: "";
										String flowrts = (f.getRights() != null) ? f
												.getRights()
												: "";
										String putxt = "Name:&nbsp;" + fname
												+ "<br>"
												+ "<font color=\"#0000ff\">"
												+ "&nbsp;Base&nbsp;URL:&nbsp;"
												+ flowid + "<br>"
								...
								Iterator itty = comproot.getChildren()
										.iterator();
								if (!itty.hasNext()) {
									_compSearchResultsRoot
											.addItem("No Results");
								}
								while (itty.hasNext()) {
									_compSearchResultsRoot
											.addItem(((WBTreeNode) itty.next())
													.getNodeItem());
								}
								itty = flowroot.getChildren().iterator();
								if (!itty.hasNext()) {
									_flowSearchResultsRoot
											.addItem("No Results");
								}
								while (itty.hasNext()) {
									_flowSearchResultsRoot
						...
						public void onFailure(Throwable caught) {
							// do some UI stuff to show failure
							_compSearchResultsRoot
									.setText("Failure Retrieving Components");
							_flowSearchResultsRoot
									.setText("Failure Retrieving Components");
							Controller.this.hideStatusBusy();
							Controller.this.setStatusMessage("Search failed.");
							Window
									.alert("AsyncCallBack Failure -- getActiveComponents:  "
											+ caught.getMessage());
						}
					};
					Controller.this.showStatusBusy();
	 ...
	 * @param compTreeRoot
	 *            TreeItem Tree root to use for building.
	 * @param rootTxt
	 *            String Root node text for this tree.
	 * @param compTreeSort
	 *            int Sort option for this component tree.
	...
	Tree buildCompTree(Tree compTree, TreeItem compTreeRoot, String rootTxt,
		...
		AsyncCallback callback = new AsyncCallback() {
			...
			public void onFailure(Throwable caught) {
				...
				_compRootTemp.setState(true);
				Window.alert("AsyncCallBack Failure -- getActiveComponents:  "
						+ caught.getMessage());
			}
		};
	 ...
	 * @param compTreeRoot
	 *            TreeItem Root node with items added.
	 */
	private void buildCompTreeAsFlatListAlpha(Set items, TreeItem compTreeRoot) {
		WBTreeNode root = new WBTreeNode();
	 ...
	 * @param compTreeRoot
	 *            TreeItem The roo of the display tree at this iteration.
	 */
	private void buildTypeTreeOfTreeItems(WBTreeNode rootNode,
			TreeItem compTreeRoot) {
		if (rootNode.hasChildren()) {
			for (Iterator itty = rootNode.getChildren().iterator(); itty
					.hasNext();) {
	 ...
	 * @param nd
	 *            Map Parent's child map.
	 * @param ecd
	 *            WBComponent This types component object.
	 */
	...
	private String getCompTreeItemPopUpText(WBComponent ecd) {
		String putxt = "Name:&nbsp;" + ecd.getName() + "<br>"
				+ "<font color=\"#0000ff\">" + "&nbsp;ID:&nbsp;" + ecd.getID()
				+ "<br>" + "&nbsp;Description:&nbsp;" + ecd.getDescription()
				+ "<br>" + "&nbsp;Creator:&nbsp;" + ecd.getCreator() + "<br>" + /*
																				 * "&nbsp;Rights:&nbsp;" +
																				 * ecd.getRights() + "<br>" +
																				 */
				"</font>";
	 ...
	 * @param flowTreeRoot
	 *            TreeItem Root node to use for flow tree.
	 * @param rootTxt
	 *            String Text for the root node for this flow tree.
			...
			public void onSuccess(Object result) {
				...
				while (itty.hasNext()) {
					...
					String putxt = "Name:&nbsp;" + f.getName() + "<br>"
							...
							+ "<br>" + "&nbsp;Creator:&nbsp;" + f.getCreator()
							+ "<br>" + "</font>";
					WBTreeItem ti = null;

					String imgFname = (pfs.get(f.getFlowID()) == null) ? "flow-tree-leaf.png"
	...
	Tree buildLocationTree(Tree locTree, TreeItem locTreeRoot, String rootTxt) {
		...
		if (locTree != null) {
			...
			locTree = new LocationTree(this, (TreeImages) GWT
					.create(LocationTreeImages.class));
		}
		if (locTreeRoot != null) {
		...
		AsyncCallback callback = new AsyncCallback() {
			...
			public void onFailure(Throwable caught) {
				...
				_locRootTemp.setState(true);
				Window.alert("AsyncCallBack Failure -- getLocations:  "
						+ caught.getMessage());
			}
		};
	 ...
	 * @param track
	 *            Set Set of compnents being placed.
	 * @param row
	 *            int Row to move component to.
	 * @param col
	 *            int Column to move component to.
	 * @return int The last row where a component was placed.
	 */
	private int plotProgenyComps(ComponentPanel cp, Set track, int row, int col) {
		int rowNum = row;
	 ...
	 * @param compID
	 *            String Component id to compare to.
	...
	private WBRepositoryQueryAsync setupRepQuery() {
		WBRepositoryQueryAsync ret = (WBRepositoryQueryAsync) GWT
				.create(WBRepositoryQuery.class);
		ServiceDefTarget endpoint = (ServiceDefTarget) ret;
		String moduleRelativeURL = GWT.getModuleBaseURL() + "WBRepositoryQuery";
	...
	private void setupComponentDragNDrop() {
		...
		// _compDragController).setBehaviorDragProxy(true);
	...
	private void resetComponentCoords() {
		for (Iterator itty = _canvasComps.iterator(); itty.hasNext();) {
			...
			cp.getComponent().getProperties().add(this.s_TopKey,
					"" + (cp.getAbsoluteTop() - _drawPan.getAbsoluteTop()));
			cp.getComponent().getProperties().add(this.s_LeftKey,
					"" + (cp.getAbsoluteLeft() - _drawPan.getAbsoluteLeft()));
		}
	}
